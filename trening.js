...
const presentationUrlFor=part=>{const presentation=part?.presentation;if(presentation?.version===1&&presentation?.sourceUrl){const source=safeUrl(presentation.sourceUrl);if(source){const returnUrl=encodeURIComponent(location.href.split("#")[0]);return source.replace(/#.*$/,"")+"#presentation="+encodeTransferPayload(presentation)+"&return="+returnUrl}}return safeUrl(part?.presentationUrl)};
